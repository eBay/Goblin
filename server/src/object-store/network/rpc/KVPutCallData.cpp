...
#include "KVPutCallData.h"

namespace goblin::objectstore::network {

KVPutCallData::KVPutCallData(
...
void KVPutCallData::proceed() {
  if (mStatus == CREATE) {
    ...
    mService->RequestPut(&mContext, &mRequest, &mResponder,
                         mCompletionQueue, mCompletionQueue, this);
  } else if (mStatus == PROCESS) {
    SPDLOG_DEBUG("debug: receive put request");
    // Spawn a new CallData instance to serve new clients while we process
    // the one for this CallData. The instance will deallocate itself as
    // part of its FINISH state.
    new KVPutCallData(mService, mCompletionQueue, mKVEngine, mObjectStore);
    mKVEngine.putAsync(mRequest, [this](const proto::Put::Response& resp) {
        mStatus = FINISH;
