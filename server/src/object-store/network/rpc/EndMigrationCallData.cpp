...
#include "EndMigrationCallData.h"

#include "../../ObjectStore.h"

namespace goblin::objectstore::network {

EndMigrationCallData::EndMigrationCallData(
...
void EndMigrationCallData::proceed() {
  if (mStatus == CREATE) {
    ...
    mService->RequestEndMigration(&mContext, &mRequest, &mResponder,
                         mCompletionQueue, mCompletionQueue, this);
  } else if (mStatus == PROCESS) {
    // Spawn a new CallData instance to serve new clients while we process
    // the one for this CallData. The instance will deallocate itself as
    // part of its FINISH state.
    new EndMigrationCallData(mService, mCompletionQueue, mKVEngine, mObjectStore);
    auto s = mObjectStore.finishMigration(mRequest.status(), mRequest.routeinfo(), mRequest.migrationtask());
    proto::EndMigration::Response resp;
    if (s.isNotLeader()) {
      mStatus = FINISH;
