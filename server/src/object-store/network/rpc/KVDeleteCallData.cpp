...
#include "KVDeleteCallData.h"

namespace goblin::objectstore::network {

KVDeleteCallData::KVDeleteCallData(
...
void KVDeleteCallData::proceed() {
  if (mStatus == CREATE) {
    ...
    mService->RequestDelete(&mContext, &mRequest, &mResponder,
                            mCompletionQueue, mCompletionQueue, this);
  } else if (mStatus == PROCESS) {
    // Spawn a new CallData instance to serve new clients while we process
    // the one for this CallData. The instance will deallocate itself as
    // part of its FINISH state.
    new KVDeleteCallData(mService, mCompletionQueue, mKVEngine, mObjectStore);
    mKVEngine.removeAsync(mRequest, [this](const proto::Delete::Response& resp) {
        mStatus = FINISH;
