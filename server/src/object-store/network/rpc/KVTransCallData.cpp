...
#include "KVTransCallData.h"

namespace goblin::objectstore::network {

KVTransCallData::KVTransCallData(
...
void KVTransCallData::proceed() {
    if (mStatus == CREATE) {
        ...
        mService->RequestTransaction(&mContext, &mRequest, &mResponder,
                             mCompletionQueue, mCompletionQueue, this);
    } else if (mStatus == PROCESS) {
        // Spawn a new CallData instance to serve new clients while we process
        // the one for this CallData. The instance will deallocate itself as
        // part of its FINISH state.
        new KVTransCallData(mService, mCompletionQueue, mKVEngine, mObjectStore);
        mKVEngine.transAsync(mRequest, [this](const proto::Transaction::Response& resp) {
            mStatus = FINISH;
