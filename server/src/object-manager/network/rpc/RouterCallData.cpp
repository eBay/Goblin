...
namespace goblin::objectmanager::network {
  ...
  void RouterCallData::proceed() {
    if (mStatus == CREATE) {
      ...
      mService->RequestRouter(&mContext, &mRequest, &mResponder,
                              mCompletionQueue, mCompletionQueue, this);
    } else if (mStatus == PROCESS) {
      // Spawn a new CallData instance to serve new clients while we process
      // the one for this CallData. The instance will deallocate itself as
      // part of its FINISH state.
      new RouterCallData(mService, mCompletionQueue, mKVEngine);
      mStatus = FINISH;

      proto::Router::Response response;
      auto &header = *response.mutable_header();
      buildHeader(header, proto::OK);
      SPDLOG_INFO("RouterCallData, begin to process request");

      module::ClusterSetManager clusterSetManager{mKVEngine};
      clusterSetManager.init(header);
      SPDLOG_INFO("RouterCallData, clusterSetManager init return code {}", header.code());
      if (header.code() != proto::OK) {
        return mResponder.Finish(response, buildStatus(header.code()), this);
      }

      clusterSetManager.handleRouteInfoRequest(header, *response.mutable_routeinfo());
      SPDLOG_INFO("RouterCallData, clusterSetManager.handleRouteInfoRequest return code {}", header.code());
      mResponder.Finish(response, buildStatus(header.code()), this);
      SPDLOG_INFO("RouterCallData, process request done");
    } else {
