...
#include "RemoveClusterCallData.h"
#include "../../module/ClusterSetManager.h"

namespace goblin::objectmanager::network {

  RemoveClusterCallData::RemoveClusterCallData(
  ...
  void RemoveClusterCallData::proceed() {
    if (mStatus == CREATE) {
      ...
      mService->RequestRemoveCluster(&mContext, &mRequest, &mResponder,
                                   mCompletionQueue, mCompletionQueue, this);
    } else if (mStatus == PROCESS) {
      // Spawn a new CallData instance to serve new clients while we process
      // the one for this CallData. The instance will deallocate itself as
      // part of its FINISH state.
      new RemoveClusterCallData(mService, mCompletionQueue, mKVEngine);
      mStatus = FINISH;

      proto::RemoveCluster_Response response;
      ...
      SPDLOG_INFO("RemoveClusterCallData, begin to process request");

      module::ClusterSetManager clusterSetManager{mKVEngine};
      clusterSetManager.init(header);
      SPDLOG_INFO("RemoveClusterCallData, clusterSetManager init return code {}", header.code());
      if (header.code() != proto::OK) {
        return mResponder.Finish(response, buildStatus(header.code()), this);
      }

      clusterSetManager.removeCluster(header, mRequest.cluster().servers());
      SPDLOG_INFO("RemoveClusterCallData, clusterSetManager.removeCluster return code {}", header.code());
      mResponder.Finish(response, buildStatus(header.code()), this);
      SPDLOG_INFO("RemoveClusterCallData, process request done");
    } else {
