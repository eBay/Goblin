...
void EventApplyLoop::run() {
  ...
  while (!mShouldExit) {
    ...
    if (mShouldRecover) {
      recoverSelf();
      appliedIndex = mLastMilestone;
      mLastMilestoneGauge.set(mLastMilestone);
    }
...
void EventApplyLoop::recoverSelf() {
  assert(mKVStore->isClearState() == true);
...
EventApplyLoop::EventApplyLoop(
    ...
    gringofts::TestPointProcessor *processor) :
    EventApplyLoop(raftEventStore, kvStore, snapshotDir) {
  mTPProcessor = processor;
}
}  /// namespace goblin::kvengine::execution
